...
class TestGoAnnotationsPage(unittest.TestCase):
    ...
    def test_table_headers(self):
        ...
        self.assertEqual(searchtextitems,
                         ['Aspect', 'Category', 'Classification Term', 'Context', 'Proteoform', 'Evidence',
                          'Inferred From', 'Reference(s)'])
    ...
    def test_aspect_sort(self):
        ...
        asp18 = driver.find_element(By.CSS_SELECTOR, '#yui-rec18 > td:nth-child(1) > div:nth-child(1)')
        asp28 = driver.find_element(By.CSS_SELECTOR, '#yui-rec28 > td:nth-child(1) > div:nth-child(1)')
        print(asp8.text)
        print(asp18.text)
        print(asp28.text)
        wait.forAjax(driver)
        self.assertEqual(asp8.text, 'Molecular Function', 'the aspect is wrong')
        self.assertEqual(asp18.text, 'Cellular Component', 'the aspect is wrong')
        self.assertEqual(asp28.text, 'Biological Process', 'the aspect is wrong')
